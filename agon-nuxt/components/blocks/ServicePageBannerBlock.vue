    ...
    <div v-if="serviceBannerData?.length">
      <div v-for="(item, index) in serviceBannerData" :key="item.id"
        ...
        <div class="text-center">
          <div v-if="item.TitleContentItem && item.TitleContentItem.title" 
          ...
          <p v-if="item.TitleContentItem && item.TitleContentItem.content" 
        ...
        <div class="flex items-center justify-center mb-[87px]"> 
          <button type="button">
            <NuxtLink
              ...
              <i><img class="ml-[7px] w-[12px] filter-white group-hover:filter-black" src="/assets/images/icons/icon-right.svg" alt="arrow right icon" /></i>
            </NuxtLink>
          </button>
        ...
        <div class="relative mx-auto max-w-[1190px]">
          <img v-if="item.image && item.image.url" class="h-full w-full object-cover rounded-2xl" :src="`${strapiBaseUrl}${item.image.url}`" alt="Agon" /> 
        </div>
      </div>
    </div>
  </template>
