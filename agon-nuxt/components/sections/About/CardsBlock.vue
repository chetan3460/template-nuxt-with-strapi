<template>
    <div v-if="cardBlockData.length" class="px-[12px] md:px-[36px] xl:px-0 mt-[70px] lg:mt-[150px]">
        <div v-for="(item, index) in cardBlockData" :key="item.id">
            ...
            <div v-if="item.ItemRepeater?.length">
                <div class="lg:flex lg:items-center gap-[30px]">
                    <div v-for="(item2, index) in item.ItemRepeater" :key="item2.id"
                        ...
                        <img v-if="item2.icon?.url" class="h-full w-full object-cover max-w-[64px]"
                            :src="getImageUrl(item2.icon.url)" alt="icon" />
                        <div>
...
const { data } = await useAsyncData(`card-block-${slug.value}`, () =>
    $fetch(`${strapiBaseUrl}/api/sitemaps?${queryParams.value}`)
);

// Extract Card Block data safely
const cardBlockData = computed(() =>
    data.value?.data?.[0]?.Blocks?.filter(block => block.__component === "page-blocks.cards-block") || []
);
</script>
