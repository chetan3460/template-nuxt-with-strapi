<template>
    <div class="px-[12px] md:px-[36px] xl:px-0 mt-[70px]">
        <div class="grid grid-cols-2 md:grid-cols-4 gap-x-[24px] lg:grid-cols-6">
            <NuxtLink
                class="transition-all duration-300 partner-item p-[15px] md:pr-[15px] xl:w-auto lg:w-[184px] hover:translate-y-[-3px]"
                to="#"><img src="/assets/images/partner-logo-1.svg" alt="partner logo"></NuxtLink>
            <NuxtLink
                class="transition-all duration-300 partner-item p-[15px] md:pr-[15px] xl:w-auto lg:w-[184px] hover:translate-y-[-3px]"
                to="#"><img src="/assets/images/partner-logo-2.svg" alt="partner logo"></NuxtLink>
            <NuxtLink
                class="transition-all duration-300 partner-item p-[15px] md:pr-[15px] xl:w-auto lg:w-[184px] hover:translate-y-[-3px]"
                to="#"><img src="/assets/images/partner-logo-3.svg" alt="partner logo"></NuxtLink>
            <NuxtLink
                class="transition-all duration-300 partner-item p-[15px] md:pr-[15px] xl:w-auto lg:w-[184px] hover:translate-y-[-3px]"
                to="#"><img src="/assets/images/partner-logo-4.svg" alt="partner logo"></NuxtLink>
            <NuxtLink
                class="transition-all duration-300 partner-item p-[15px] md:pr-[15px] xl:w-auto lg:w-[184px] hover:translate-y-[-3px] md:col-start-2 lg:col-start-auto"
                to="#"><img src="/assets/images/partner-logo-5.svg" alt="partner logo"></NuxtLink>
            <NuxtLink
                class="transition-all duration-300 partner-item p-[15px] md:pr-[15px] xl:w-auto lg:w-[184px] hover:translate-y-[-3px] md:col-start-3 lg:col-start-auto"
                to="#"><img src="/assets/images/partner-logo-6.svg" alt="partner logo"></NuxtLink>
        </div>
    </div>
</template>